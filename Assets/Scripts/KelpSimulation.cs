...
public class KelpSimulation : MonoBehaviour 
{
    [Header("Kelp")]
    ...
    [SerializeField] private GameObject topSegmentMeshPrefab; 

    private List<GameObject> segmentMeshes = new ();
    private GameObject topSegmentMesh; 
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < numberOfKelpSegments; i++)
        {
            ...
            // Unique mesh goes on the top segment (last one)
            if (i == numberOfKelpSegments - 1 && topSegmentMeshPrefab != null)
            {
                topSegmentMesh = Instantiate(topSegmentMeshPrefab, kelpStartPosition, Quaternion.identity, transform);
            }
            else if (i < numberOfKelpSegments - 1 && kelpSegmentMeshPrefab != null)
            {
    ...
    private void Update()
    {
        if (topSegmentMesh != null)
        {
            int topIndex = numberOfKelpSegments - 1;
            topSegmentMesh.transform.position = kelpSegments[topIndex].CurrentPosition;

            if (topIndex > 0)
            {
                Vector3 dir = kelpSegments[topIndex].CurrentPosition - kelpSegments[topIndex - 1].CurrentPosition;
                if (dir.sqrMagnitude > 0.0001f)
                    topSegmentMesh.transform.rotation = Quaternion.LookRotation(dir);
            }
        } 
